...
const FireEffect = () => {
  ...
  useEffect(() => {
    ...
    programRef.current = createProgram(gl, vertexShader, fragmentShader); // Assign program to ref

    // gl.useProgram(program); // Moved outside the render function
    ...
    const dateUniformLocation = gl.getUniformLocation(
      programRef.current,
      "date"
    ); // Use programRef.current
    const pointerCountUniformLocation = gl.getUniformLocation(
      programRef.current,
      "pointerCount"
    ); // Use programRef.current
    const pointersUniformLocation = gl.getUniformLocation(
      programRef.current,
      "pointers"
    ); // Use programRef.current
    ...
    const render = (time) => {
      gl.useProgram(programRef.current); // Use programRef.current here
      ...
      gl.uniform1i(pointerCountUniformLocation, 1);
      gl.uniform3fv(pointersUniformLocation, [mouseX, mouseY, 0]);
