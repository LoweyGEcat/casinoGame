...
function startGame(game) {
  ...
  game.players.forEach((player, index) => {
    ...
    player.points = player.hand.reduce(
      (total, card) => total + calculateCardPoints(card),
      0
    );
  });
...
function handleDraw(game, fromDeck, meldIndices = []) {
  ...
  if (!fromDeck && game.discardPile.length > 0) {
    ...
    if (meldIndices.length > 0) {
      const meldCards = [
        ...meldIndices.map((i) => currentPlayer.hand[i]),
        drawnCard,
      ];
...
function handleDiscard(game, cardIndex) {
  ...
  currentPlayer.points = currentPlayer.hand.reduce(
    (total, card) => total + calculateCardPoints(card),
    0
  );
...
function handleAutoSort(game, playerIndices, requestingPlayerId) {
  playerIndices.forEach((playerId) => {
    ...
    melds.forEach((meld) =>
      meld.sort((a, b) => rankToNumber(a.rank) - rankToNumber(b.rank))
    );
